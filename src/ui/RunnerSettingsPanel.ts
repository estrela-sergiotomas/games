...
export class RunnerSettingsPanel {
  ...
  private settings: SettingDef[] = [
    ...
    { label: 'Limite p/ Acelerar', key: 'speedThreshold', get: () => RunnerSettings.speedThreshold, set: v => RunnerSettings.speedThreshold = v, min: 2, max: 30, step: 1, format: v => v.toFixed(0) + 'x', desc: 'Multiplicador onde velocidade sobe' },
    { label: 'Mult./Distancia', key: 'multDist', get: () => RunnerSettings.multiplierPerDistance, set: v => RunnerSettings.multiplierPerDistance = v, min: 0.0002, max: 0.01, step: 0.0002, format: v => v.toFixed(4), desc: 'Quanto o mult. sobe por distancia' },
    { label: 'Mult./Moeda', key: 'multCoin', get: () => RunnerSettings.multiplierPerCoin, set: v => RunnerSettings.multiplierPerCoin = v, min: 0.01, max: 0.2, step: 0.01, format: v => v.toFixed(2), desc: 'Quanto o mult. sobe por moeda' },
